  ...
  <>
    {!currentUser ? (
      <a href="/login">Go login</a>
          ...
          <select name="lieu" value={myLocation} onChange={(e) => setMyLocation(e.currentTarget.value)}>
          ...
          <select name="type" value={myLockerType} onChange={(e) => setMyLockerType(e.currentTarget.value)}>
          <option value="" selected>...</option>
            {lockerType.map(item => (
                <option key={item.id} value={item.id}>
                  {item.name}
          ...
          <select name="locker" value={myLocker} onChange={(e) => setMyLocker(e.currentTarget.value)}>
          <option value="" selected>...</option>
            {locker.map(item => (  
              (item.locationId == myLocation && item.locker_type_id == myLockerType ? (<option key={item.id} value={item.id}>
                {item.name}
              </option>) : (console.log('nul'))            
          ...
          <input name="start" type="datetime-local" min="2020-06-01T08:30" onChange={(e)=> setMyDateDepot(moment(e.currentTarget.value).format('YYYY-MM-DD HH:mm:ss'))} />
          <label for="end">Date de retrait</label>
          <input name="end" type="datetime-local" min="2020-06-01T08:30" onChange={(e)=> setMyDateRetrait(moment(e.currentTarget.value).format('YYYY-MM-DD HH:mm:ss'))} />
